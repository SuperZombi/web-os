const Window = ({ win, children }) => {
	...
	const [state, setState] = React.useState({
		w: win?.width  || window.innerWidth  / 2,
		h: win?.height || window.innerHeight / 2,
		x: win?.x || Math.floor((window.innerWidth  - (win?.width  || window.innerWidth  / 2)) / 2),
		y: win?.y || Math.floor((window.innerHeight - (win?.height || window.innerHeight / 2)) / 2),
	})
	...
	return (
		<div
			className={`absolute bg-${theme === 'dark' ? 'gray-800' : 'gray-100'}/50 backdrop-blur-lg rounded-lg border border-${theme === 'dark' ? 'gray-600' : 'gray-400'}/50 shadow-lg overflow-hidden`}
			style={{ width: state.w, height: state.h, left: state.x, top: state.y }}
		>
			<div
				className={`bg-${theme === 'dark' ? 'gray-400' : 'white'}/25 border-b border-${theme === 'dark' ? 'gray-600' : 'gray-400'}/50 h-8 px-2 cursor-move select-none
