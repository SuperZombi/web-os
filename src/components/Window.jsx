const Window = ({ win, children, theme, accentColor }) => {
	...
	return (
		<div ref={rootRef} onMouseDown={activate}
			className={`window absolute rounded-xl border shadow-2xl overflow-hidden ${show ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'} transition-opacity transition-transform duration-300 ease-out ${theme === 'dark' ? 'bg-slate-900/65 border-white/15' : 'bg-white/75 border-slate-300/80'} backdrop-blur-xl`}
			style={{ width: state.w, height: state.h, left: state.x, top: state.y, boxShadow: `0 20px 45px ${theme === 'dark' ? '#0008' : '#33415522'}` }}>
			{showHeader && (<div className={`h-10 px-3 select-none flex items-center justify-between relative border-b ${dragabble && "cursor-move"} ${theme === 'dark' ? 'border-white/10 bg-white/5' : 'border-black/10 bg-white/40'}`} onMouseDown={dragabble ? ((e) => startInteraction(e, 'move')) : null}>
				<h3 className={`text-sm font-semibold ${theme === 'dark' ? 'text-white' : 'text-slate-800'}`}>{win?.name}</h3>
				<button className="h-6 w-6 rounded-full text-white text-xs" style={{ backgroundColor: accentColor }} onClick={() => {window.WindowManager.closeWindow(win.id)}}>×</button>
			</div>)}
